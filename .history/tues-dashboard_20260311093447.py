...
def load_and_process_data():
    # 1. Load Data (Only the Traceable Mothers Dataset)
    ...
    # 4. Filter dataset to matched sites
    df_m_opt = df_mothers[
        df_mothers["Facility_Clean"].isin(fac_to_optsite.keys())
    ].copy()
    df_m_opt["Optimized_Site"] = df_m_opt["Facility_Clean"].map(fac_to_optsite)

    # 5. Format Dates for infant filtering
    df_m_opt["infant_date_of_birth"] = pd.to_datetime(
        df_m_opt["infant_date_of_birth"], format="%d/%m/%Y", errors="coerce"
    )
    df_m_opt["date_of_delivery"] = pd.to_datetime(
        df_m_opt["date_of_delivery"], format="%d/%m/%Y", errors="coerce"
    )
    # Combine DOB and Delivery date just in case one is blank
    df_m_opt["infant_dob_merged"] = df_m_opt["infant_date_of_birth"].combine_first(
        df_m_opt["date_of_delivery"]
    )
    ...
    # Part 2: Infant Cohort strictly born between Oct 2022 and Sep 2023 (and mother is HIV+)
    df_infant_cohort = df_m_opt[
        (df_m_opt["infant_dob_merged"] >= "2022-10-01")
        & (df_m_opt["infant_dob_merged"] <= "2023-09-30")
    ].copy()
    ...
    infant_summary = (
        ...
        .agg(
            Total_Exposed=("child_person_id", "nunique"),
            Untraced=(
                "child_person_id",
                lambda x: 0,
            ),  # 0 because this is strictly the Traceable dataset
    ...
    infant_summary["Traced"] = infant_summary["Total_Exposed"]
    ...
    babies_born = df_m_pos["child_person_id"].nunique()
    ...
    # Part 2 Metrics (Oct 2022 - Sep 2023)
    exp_facilities = df_infant_cohort["Facility_Clean"].nunique()
    total_exposed = len(df_infant_cohort)
    traced = total_exposed  # 100% traced
    untraced = 0
    traced_pct = 100.0 if total_exposed else 0
    untraced_pct = 0.0
    ...
    # Generate narratives (Removed date filtering text from Part 1)
...
col1.metric("Total HIV+ Women (ANC All Time)", f"{total_hiv_pos:,}")
col2.metric(
...
col4.metric("Total Exposed Infants (Oct 2022-Sep 2023)", f"{total_exposed:,}")
...
with col_part2_1:
    ...
    df["Traced_Display"] = df["Total_Exposed"]
...
st.markdown(
    "This table isolates the specific cohort of HIV-exposed infants to assess how many were linked to maternal clinical records, and their final HIV status at 24 months. **Note: Because this analysis is drawn exclusively from the Traceable Mothers dataset, 100% of infants are traced.**"
)
