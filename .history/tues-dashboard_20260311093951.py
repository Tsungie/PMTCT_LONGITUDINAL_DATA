...
def load_and_process_data():
    ...
    # Part 2: Infant Cohort strictly born between Oct 2022 and Sep 2023 (Using infant_date_of_birth)
    ...
    anc_summary = (
        ...
        .agg(
            ...
            Babies_Born=(
                "child_person_id",
                "nunique",
            ),  # This represents Total Exposed Infants All Time per site
    ...
    traced = (
        total_exposed_cohort  # 100% traced since it's from traceable mother dataset
    )
    untraced = 0
...
col1, col2, col3, col4, col5 = st.columns(5)  # Expanded to 5 columns

total_hiv_pos = df["Total_HIV_Positive"].sum()
total_unaware = df["Unaware_Prior"].sum()
total_aware = df["Aware_Prior"].sum()
total_exposed_all_time = df["Babies_Born"].sum()  # Babies born to HIV+ women all time
total_exposed_cohort = df["Total_Exposed_Cohort"].sum()  # Cohort specific
...
col5.metric("Exposed Infants (Oct '22 - Sep '23)", f"{total_exposed_cohort:,}")
...
with col_chart1:
    ...
    st.plotly_chart(
        fig_anc, use_container_width=True, key="chart_anc_aware"
    )  # Added unique key
...
with col_chart2:
    ...
    st.plotly_chart(
        fig_inf, use_container_width=True, key="chart_anc_tested"
    )  # Added unique key
...
with col_part2_1:
    ...
    st.plotly_chart(
        fig_linkage, use_container_width=True, key="chart_part2_linkage"
    )  # Added unique key
...
with col_part2_2:
    ...
    st.plotly_chart(
        fig_outcomes, use_container_width=True, key="chart_part2_outcomes"
    )  # Added unique key
