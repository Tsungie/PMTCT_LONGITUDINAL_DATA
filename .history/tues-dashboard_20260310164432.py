...
def load_and_process_data():
    ...
    df_mothers = pd.read_csv("uploads/DATA_SET_WITH_TRACE_OF_THE_MOTHER.csv", encoding="latin1")
...
# ==========================================
# PART 2: INFANT COHORT & MATERNAL LINKAGE
# ==========================================
st.divider()
st.header("Part 2: HIV-Exposed Infant Linkage & Outcomes")
st.markdown(
    "Focusing on the cohort of HIV-exposed infants born between October 2022 and September 2023 to assess maternal linkage and 24-month testing outcomes."
)

# Using the infant summary dataframe we built earlier (df)
col_part2_1, col_part2_2 = st.columns(2)

# Chart 1: Maternal Linkage
with col_part2_1:
    st.subheader("Maternal Linkage: Traced vs Untraced")

    # Calculate traced vs untraced based on the 'DATA SET WITH NO TRACEABLE MOTHERS.csv'
    # We know from the main script that 100% of this specific infant dataset are untraced
    # so Total_Exposed = Untraced. We'll plot Untraced vs Traced.
    df["Untraced"] = df["Total_Exposed"]
    df["Traced"] = 0

    df_melt_linkage = df[df["Total_Exposed"] > 0].melt(
        id_vars="Optimized_Site",
        value_vars=["Untraced", "Traced"],
    ...
    fig_linkage = px.bar(
        ...
        orientation="h",
        color_discrete_map={"Untraced": "#ff9999", "Traced": "#66b3ff"},
        labels={"Count": "Number of Infants", "Optimized_Site": "Facility"},
    )
    st.plotly_chart(fig_linkage, use_container_width=True)

# Chart 2: Final Outcomes
with col_part2_2:
    st.subheader("Final HIV Outcomes at 24 Months")

    # Calculate unknown outcomes
    df["Final_Negative"] = (
        0  # Based on the logic that 100% were positive in this dataset
    )
...
st.markdown("### Part 2 Data Table")
st.dataframe(
    df[df["Total_Exposed"] > 0][
        [
            "Optimized_Site",
            "Total_Exposed",
            "Untraced",
