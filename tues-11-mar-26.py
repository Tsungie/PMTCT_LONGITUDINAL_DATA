...
# ==========================================
print("\n--- MISSING SITES CHECK ---")
# Get the unique facilities that successfully made it into the ANC dataset
found_anc_sites = df_m_opt['Facility_Clean'].unique().tolist()

missing_count = 0
for site in opt_sites:
    # Check if the optimized site exists in the matched data (using our fuzzy logic)
    match_found = False
    for f in found_anc_sites:
        if site in f or f in site or difflib.SequenceMatcher(None, site, f).ratio() > 0.8:
            match_found = True
            break
            
    if not match_found:
        print(f"MISSING: '{site.title()}' was not found in the ANC dataset.")
        missing_count += 1

if missing_count == 0:
    print("All 20 sites are present!")
print("---------------------------\n")
